...
class Shape {
 public:
  ...
  virtual std::shared_ptr<Shape> getChild(std::size_t)
  {
    return nullptr;
  }
...
class Picture : public Shape { // 包含多个单体对象的组合对象
 public:
  ...
  void add(const std::shared_ptr<Shape>& s) override
  {
  ...
  {
    if (const auto it = std::find(std::begin(shapes), std::end(shapes), s); it != std::end(shapes))
    {
      shapes.erase(it);
    }
  }
